...
public sealed class MySqlExporter : ISqlExporter
{
    private readonly OutputOptions _outputOptions;
    private readonly SqlLoader _sqlLoader;
    private readonly string _root;

    public MySqlExporter(OutputOptions outputOptions, SqlLoader sqlLoader, string root)
    {
    ...
    {
        var sqlPath = PathUtilities.ResolvePath(_root, _outputOptions.MysqlSqlPath);
        ...
        await using (var writer = new StreamWriter(sqlPath, false, new UTF8Encoding(false)))
        {
            ...
            await writer.WriteLineAsync(_sqlLoader.Load("mysql_estados.sql"));
            await writer.WriteLineAsync(_sqlLoader.Load("mysql_municipios.sql"));
            await writer.WriteLineAsync(_sqlLoader.Load("mysql_localidades.sql"));
